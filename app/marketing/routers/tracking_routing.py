...
from app.marketing.models import Interaction, Contact, BusinessPhoneLine
...
from app.admin.services.twilio_service import TwilioService
from app.marketing.schemas.tracking_routing import (
    CallLogItem,
    CallLogResponse,
    DepartmentSummaryItem,
    CampaignSummaryItem,
    PhoneLineSummaryItem,
...
@router.get("/tracking-number-summary", response_model=list[PhoneLineSummaryItem])
async def tracking_number_summary(
    business_id: UUID = Query(...),
    current_user_id: UUID = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """Aggregate call data by phone line with department attribution.

    Groups all inbound calls by the phone number that was called,
    then breaks down how many were routed to each department. This is
    the core marketing attribution report: which phone lines generate
    calls and where those calls end up.
    """
    # Load all phone lines for this business
    ...
    phone_lines = {pl.twilio_number: pl for pl in tn_result.scalars().all()}

    # Load all call interactions
    result = await db.execute(
        select(Interaction).where(
            Interaction.business_id == business_id,
            Interaction.type == "call",
    ...
    # Group by the "to" number (the tracking number that was called)
    number_data: dict[str, dict] = defaultdict(
        lambda: {
            "total": 0, "completed": 0, "followup": 0, "dropped": 0,
            "durations": [], "departments": defaultdict(int),
        }
    )
    ...
    for r in rows:
        ...
        dept = meta.get("routed_to_department") or meta.get("department_context")
        if dept:
            d["departments"][dept] += 1

    items = []
    for number, d in sorted(number_data.items(), key=lambda x: -x[1]["total"]):
        pl = phone_lines.get(number)
        durations = d["durations"]
        avg_dur = sum(durations) / len(durations) if durations else 0

        dept_breakdown = [
            DepartmentAttribution(department=dept, call_count=cnt)
            for dept, cnt in sorted(d["departments"].items(), key=lambda x: -x[1])
        ]

        items.append(
            PhoneLineSummaryItem(
                id=str(pl.id) if pl else None,
                tracking_number=number,
                twilio_number_sid=pl.twilio_number_sid if pl else None,
                friendly_name=pl.friendly_name if pl else None,
                campaign_name=pl.campaign_name if pl else "Unknown",
                channel=pl.channel if pl else None,
                line_type=pl.line_type if pl else "tracking",
                shaken_stir_status=pl.shaken_stir_status if pl else "unverified",
                total_calls=d["total"],
                completed_count=d["completed"],
                followup_count=d["followup"],
                dropped_count=d["dropped"],
                avg_duration_s=round(avg_dur, 1),
                department_breakdown=dept_breakdown,
            )
        )

    # Also include phone lines with zero calls (newly provisioned)
    seen_numbers = {item.tracking_number for item in items}
    for number, pl in phone_lines.items():
        if number not in seen_numbers:
            items.append(
                PhoneLineSummaryItem(
                    id=str(pl.id),
                    tracking_number=number,
                    twilio_number_sid=pl.twilio_number_sid,
                    friendly_name=pl.friendly_name,
                    campaign_name=pl.campaign_name,
                    channel=pl.channel,
                    line_type=pl.line_type,
                    shaken_stir_status=pl.shaken_stir_status,
                )
            )
...
class TestGreetingRequest(BaseModel):
    """Payload for triggering a test call that plays the full call flow."""
    phone_number: str  # E.164 phone number to call


@router.post("/test-greeting")
async def test_greeting_call(
    payload: TestGreetingRequest,
    business_id: UUID = Query(...),
    current_user_id: UUID = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """
    Initiate a Twilio call that plays the full IVR flow:
      1. Greeting (what callers hear first)
      2. Brief pause (simulating the caller speaking)
      3. Hold message (what plays while connecting)
    Used for previewing the complete caller experience.
    """
    ...
    to_number = f"+{digits}"
    if len(digits) < 11:
        raise HTTPException(status_code=400, detail="Invalid phone number. Please enter a valid 10-digit US number.")

    # Get Twilio credentials
    creds = await twilio_service.get_credentials(db, business_id)
    if not creds:
        raise HTTPException(status_code=400, detail="Twilio is not connected for this business.")

    # Get business name for {company_name} interpolation
    biz_result = await db.execute(
        select(Business).where(Business.id == business_id)
    )
    business = biz_result.scalar_one_or_none()
    company_name = business.name if business else "your company"
    ...
    greeting_text = phone_settings.greeting_text if phone_settings else DEFAULT_GREETING
    hold_message_text = phone_settings.hold_message if phone_settings else DEFAULT_HOLD_MESSAGE
    voice_name = phone_settings.voice_name if phone_settings else "Google.en-US-Chirp3-HD-Aoede"

    if not greeting_text:
        greeting_text = DEFAULT_GREETING
    if not hold_message_text:
        hold_message_text = DEFAULT_HOLD_MESSAGE

    # Interpolate {company_name} and sanitize
    ...
    hold_msg = hold_message_text.replace("{company_name}", safe_name)
    hold_msg = hold_msg.replace("&", "and").replace("<", "").replace(">", "")

    # Build TwiML that plays the full call flow:
    # 1. Greeting → 2. Pause (simulates caller speaking) → 3. Hold message → End
    twiml = (
        f'<?xml version="1.0" encoding="UTF-8"?>'
        f"<Response>"
        f'<Say voice="{voice_name}">{greeting}</Say>'
        f'<Pause length="3"/>'
        f'<Say voice="{voice_name}">{hold_msg}</Say>'
        f'<Pause length="1"/>'
        f'<Say voice="{voice_name}">That was a preview of your full call flow. Goodbye.</Say>'
        f"</Response>"
    )

    # Find a "from" number — use mainline or first available phone line
    tn_result = await db.execute(
        select(BusinessPhoneLine).where(
            BusinessPhoneLine.business_id == business_id,
            BusinessPhoneLine.active == True,
    ...
    phone_lines_list = tn_result.scalars().all()
    # Prefer the mainline number, else first available
    from_number = None
    for pl in phone_lines_list:
        if pl.line_type == "mainline":
            from_number = pl.twilio_number
            break
    if not from_number and phone_lines_list:
        from_number = phone_lines_list[0].twilio_number
    if not from_number:
        raise HTTPException(
            status_code=400,
            detail="No phone numbers available to place the test call from.",
        )

    # Initiate the call via Twilio REST API
    print(f"[TEST-GREETING] Placing call: from={from_number} to={to_number}")
    try:
        ...
        print(f"[TEST-GREETING] Call created: SID={call.sid} status={call.status}")

        # Poll briefly to catch early failures (busy, no-answer, etc.)
        import time
        ...
        for _ in range(6):  # Poll for up to ~6 seconds
            ...
            print(f"[TEST-GREETING] Poll: status={final_status}")
            if final_status in ("in-progress", "completed", "busy", "failed", "no-answer", "canceled"):
                break
        ...
        if final_status == "busy":
            return {
                ...
                "call_sid": call.sid,
                "from": from_number,
                "message": "Your phone returned a busy signal. This usually means your carrier (e.g. T-Mobile Scam Shield) is blocking the call. Try adding the mainline number to your contacts and disabling spam filtering.",
            }
        elif final_status in ("failed", "no-answer", "canceled"):
            return {
                "status": final_status,
                "call_sid": call.sid,
                "from": from_number,
                "message": f"Call {final_status}. Check that your phone is on and can receive calls from {from_number}.",
            }

        return {"status": "calling", "call_sid": call.sid, "from": from_number}
    except Exception as e:
        print(f"[TEST-GREETING] FAILED: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to place test call: {e}"
        )


@router.get("/debug-call/{call_sid}")
async def debug_call_status(
    call_sid: str,
    business_id: UUID = Query(...),
    current_user_id: UUID = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """Debug endpoint: look up a Twilio call by SID and return its status."""
    creds = await twilio_service.get_credentials(db, business_id)
    if not creds:
        raise HTTPException(status_code=400, detail="Twilio not connected")
    try:
        client = twilio_service._get_client(creds)
...
@router.get("/shaken-stir/status")
async def get_shaken_stir_status(
    business_id: UUID = Query(...),
    current_user_id: UUID = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """Get SHAKEN/STIR verification status for a business.

    Returns which numbers have "A" attestation (verified) and which don't.
    """
    from app.admin.services.trust_hub_service import trust_hub_service

    try:
        status = await trust_hub_service.get_status(db, business_id)
        return status
    except Exception as e:
...
@router.post("/shaken-stir/setup")
async def setup_shaken_stir(
    business_id: UUID = Query(...),
    current_user_id: UUID = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """Auto-setup SHAKEN/STIR for a business.

    Finds the existing Customer Profile, creates a Trust Product if needed,
    assigns all active tracking numbers, and submits for review.
    """
    from app.admin.services.trust_hub_service import trust_hub_service

    try:
        result = await trust_hub_service.auto_setup(db, business_id)
        return result
    except Exception as e:
        logger.error(f"[SHAKEN/STIR] Auto-setup failed: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"SHAKEN/STIR setup failed: {e}",
        )


@router.post("/shaken-stir/assign-number")
async def assign_number_to_shaken_stir(
    business_id: UUID = Query(...),
    phone_number_sid: str = Query(...),
    current_user_id: UUID = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """Assign a single phone number to the SHAKEN/STIR Trust Product.

    Use this when a new tracking number is added to keep it verified.
    """
    from app.admin.services.trust_hub_service import trust_hub_service

    status = await trust_hub_service.get_status(db, business_id)
    if not status["trust_product_sid"]:
        raise HTTPException(
            status_code=400,
            detail="No SHAKEN/STIR Trust Product found. Run setup first.",
        )

    ok = await trust_hub_service.assign_number(
        db, business_id, status["trust_product_sid"], phone_number_sid
    )
    if ok:
        return {"status": "assigned", "phone_number_sid": phone_number_sid}
    else:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to assign {phone_number_sid} to SHAKEN/STIR product.",
        )


@router.post("/phone-lines/{line_id}/verify")
async def verify_phone_line(
    line_id: UUID,
    business_id: UUID = Query(...),
    current_user_id: UUID = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """Verify a phone line for SHAKEN/STIR 'A' attestation.

    Full flow:
      1. Ensure Trust Product exists (auto-setup if needed)
      2. Assign this specific number to the Trust Product
      3. Update the shaken_stir_status column on business_phone_lines
    """
    from app.admin.services.trust_hub_service import trust_hub_service, TrustHubError

    # Look up the phone line
    result = await db.execute(
        select(BusinessPhoneLine).where(
    ...
    phone_line = result.scalar_one_or_none()
    if not phone_line:
        raise HTTPException(status_code=404, detail="Phone line not found")

    if not phone_line.twilio_number_sid:
        raise HTTPException(
            status_code=400,
            detail="Phone line has no Twilio SID — it may have been entered manually.",
        )

    try:
        # Step 1: Ensure trust product exists
        status = await trust_hub_service.get_status(db, business_id)
        if not status["has_trust_product"]:
            # Auto-setup creates profile + trust product + submits for review
            status = await trust_hub_service.auto_setup(db, business_id)

        trust_product_sid = status.get("trust_product_sid")
        if not trust_product_sid:
            phone_line.shaken_stir_status = "unverified"
            await db.commit()
            raise HTTPException(
                status_code=400,
                detail="Could not create SHAKEN/STIR Trust Product. "
                       "Check that your Twilio Business Profile is complete.",
            )

        # Step 2: Assign this number to the trust product
        ok = await trust_hub_service.assign_number(
            db, business_id, trust_product_sid, phone_line.twilio_number_sid
        )

        # Step 3: Update DB status based on trust product state
        product_status = status.get("trust_product_status", "")
        if product_status == "twilio-approved" and ok:
            phone_line.shaken_stir_status = "verified"
        else:
            phone_line.shaken_stir_status = "pending"

        await db.commit()

        return {
            "status": phone_line.shaken_stir_status,
            "trust_product_status": product_status,
            "phone_line_id": str(line_id),
            "twilio_number": phone_line.twilio_number,
        }

    except TrustHubError as e:
        phone_line.shaken_stir_status = "unverified"
        await db.commit()
        raise HTTPException(status_code=500, detail=str(e))
