"""
Anthropic Service — calls Claude models via Azure AI Foundry.
...
Each employee call loads the system prompt from DB and sends a single
messages.create() call to the configured Claude model deployment.

Usage:
    from app.core.services.anthropic_service import anthropic_service

    result = await anthropic_service.call_employee(
    ...
    result = await anthropic_service.call_assistant(
        business_id=uuid,
        message="Help me set up my business profile",
        db=session,
    )
"""
...
# Map model_tier values → Foundry deployment names
...
class AnthropicServiceNotReady(AnthropicServiceError):
    """Raised when the Anthropic service is not configured or reachable."""
    pass


# Backward-compatibility aliases (callers that catch ClaudeCliError still work)
ClaudeCliError = AnthropicServiceError
ClaudeCliNotReady = AnthropicServiceNotReady
ClaudeCliTokenExpired = AnthropicServiceError  # No longer a distinct case


# ── Module-level helper (imported directly by some callers) ─────────────────
...
class AnthropicService:
    # Kept for call signature compatibility — tool names are no longer passed
    # to the model (SDK mode has no Bash/WebSearch tools). Preserved so callers
    # that construct tool lists don't error.
    ...
    def __init__(self) -> None:
        # Token provider reused across requests (handles caching internally)
    ...
    def _normalize_model(model: str) -> str:
        """Strip date suffixes like -20251001 for Foundry deployment names."""
    ...
    async def _call_model(
        ...
        db=None,
        business_id=None,
        allowed_tools=None,  # kept for compat — tool access removed in SDK mode
    ) -> str:
        """Call Claude via Azure AI Foundry Messages API using httpx + DefaultAzureCredential."""
        use_model = self._normalize_model(
            model or MODEL_MAP.get(settings.foundry_default_model, settings.foundry_default_model)
        )
        logger.info(f"[{label}] model={use_model}")
        try:
            token = self._token_provider()
            url = f"{settings.foundry_endpoint}/anthropic/v1/messages"
            payload = {
                "model": use_model,
                "max_tokens": 4096,
    ...
    # Alias so callers using _run_claude() keep working
    ...
    async def load_employee_from_db(
        ...
        """
        Load employee + system_prompt from DB.
        ...
        stmt = (
            ...
            .order_by(Employee.business_id.desc())  # business-scoped first
        )
    ...
    def build_tool_instructions(business_id: UUID) -> str:
        ...
        base = AnthropicService.INTERNAL_API_BASE
        bid = str(business_id)
...
If you learn something genuinely new during this task, document it:
...
"""
    ...
    async def call_assistant(
        ...
        """Quick call to a named employee — used for onboarding, main assistant chat."""
        if not db:
            raise AnthropicServiceError("Cannot call employee: DB session required")
        ...
        try:
            ...
            raise AnthropicServiceError(f"Employee ({file_stem}) not found in DB: {e}")

        if system_prompt is None:
            raise AnthropicServiceError(f"Employee ({file_stem}) has no system_prompt in DB")
    ...
    async def call_employee(
        ...
        **kwargs,
        ...
        """Call an employee with full business context and execution rules."""
        if not db:
            raise AnthropicServiceError(
        ...
        try:
            ...
            raise AnthropicServiceError(f"Employee {employee_id} not found in DB: {e}")

        if system_prompt is None:
            raise AnthropicServiceError(f"Employee {employee_id} has no system_prompt in DB")
    ...
    async def call_employee_inline(
        ...
        """Lighter delegation from the assistant's chat to a named employee."""
        if not db:
            raise AnthropicServiceError(
    ...
    async def chat(
        ...
        """Generic chat with a system prompt — used by callers that build their own prompts."""
        return await self._call_model(
            system_prompt=system_prompt,
            message=message,
            db=db,
    ...
    async def startup_check(self) -> dict:
        """Run at server startup to verify Foundry credentials are available."""
        try:
            self._token_provider()  # triggers credential resolution
            return {
                "ready": True,
                "provider": "azure_foundry",
                "endpoint": settings.foundry_endpoint,
                "model": settings.foundry_default_model,
                "message": "Azure AI Foundry credentials resolved",
            }
        except Exception as e:
            return {
                "ready": False,
...
claude_cli = anthropic_service  # backward-compat alias for lazy importers
